...
import PullInJobPanel from "./pull-in-job-panel";
import AddPhotosButton from "./add-photos-button";
import ResyncServiceTypeButton from "./resync-service-type-button";
...
export default async function ImportsPage({
  ...
  const { data: projects } = await supabase
    .from("projects")
    .select("id, customer_name, city, state, service_type, service_confirmed, publication_status, companycam_project_id, project_description, created_at")
    .not("companycam_project_id", "is", null)
    .order("created_at", { ascending: false });

  // For the "add to an existing draft" dropdown in the pull-in panel below —
  // any non-published project, not just ones already linked to CompanyCam.
  ...
  const needsAttention = (projects ?? []).filter(
    (p) =>
      !p.service_type ||
      !p.service_confirmed ||
      hasNeedsReviewFlag(p.project_description) ||
      (photoCountByProject.get(p.id) ?? 0) === 0
  ).length;
    ...
    <main className="mx-auto max-w-5xl px-4 py-10">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 flex-wrap items-start gap-2">
          <PullInJobPanel draftProjects={draftProjects ?? []} />
          <ClearAllButton count={projectIds.length} />
        </div>
      </div>
      ...
      <div className="mt-6 space-y-4">
        {(projects ?? []).map((p) => {
          ...
          const serviceTypeMissing = !p.service_type || !p.service_confirmed;
          const needsReview = serviceTypeMissing || hasNeedsReviewFlag(p.project_description);
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    ...
                    <div className="text-sm text-slate-500">
                      {p.city}, {p.state} —{" "}
                      {p.service_type ? p.service_type.replace(/_/g, " ") : "no service type yet"}
                    </div>
                  </div>
                  <span
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  ...
                  {!published && needsReview && <ResyncServiceTypeButton projectId={p.id} />}
                  {!published && <DeleteButton projectId={p.id} label={p.customer_name} />}
                </div>

                {!published && (
                  <div className="mt-3">
                    <AddPhotosButton projectId={p.id} label={p.customer_name} />
                  </div>
                )}
              </div>
            </div>

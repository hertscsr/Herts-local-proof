...
export default function ManualImportClient({ draftProjects }: { draftProjects: DraftProject[] }) {
  ...
  const [targetProjectId, setTargetProjectId] = useState<string>(draftProjects[0]?.id ?? "");
    ...
    try {
      const res = await fetch("/api/companycam/manual-import", {
        ...
        body: JSON.stringify({
          ...
          targetProjectId: destination === "existing" ? targetProjectId : null,
        }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error ?? "Import failed");
      setImportResult(`Imported ${json.result.importedCount} of ${json.result.requested} photo(s).`);
      router.refresh();
    } catch (e) {

...
export default function EditProjectForm({ project }: Props) {
  ...
  async function save() {
    ...
    try {
      if (!serviceType) {
        setError("Pick a service type before saving — it can't be published without one.");
        setSaving(false);
        return;
      }
      const res = await fetch(`/api/projects/${project.id}`, {
        ...
        body: JSON.stringify({ ...basics, ...fields, faq, service_type: serviceType, service_confirmed: true }),
      });
    ...
    <div className="mt-6 space-y-5">
      ...
      <div>
        ...
        <p className="mt-1 text-xs text-slate-500">
          {serviceType
            ? "Pulled in automatically from the CompanyCam label, or set by hand. Saving confirms it and clears the “needs review” warning."
            : "CompanyCam didn't have a label that matches a known service type — pick one before this can be published."}
        </p>
      </div>
